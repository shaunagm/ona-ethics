...
<div id="question" class="main-column float">
	...
	<? if ( $steps[get_the_ID()]['parent'] == true && get_the_ID() != 5) { 
		$goto = -1;
	...
	<form class="ethics-question checkbox" method="post" action="<?=$target?>">
		...
		<? if ($information){ 
			...
			foreach ($options as $option){ ?>
				...
				<? if(($key = array_search($option, $question_data)) !== false) { unset($question_data[$key]); }
			} 
			...
			<textarea rows="5" name="question_response[]"><? if (isset($question_data[0]) && $question_data[0] != 'skipped') { foreach($question_data as $q){ echo $q."\n"; } } ?></textarea>
		<? } else { ?>
